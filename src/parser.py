...
def parse_employees(fn, chave_coleta, categoria):
    ...
    for row in fn:
        ...
        function = row[2]
        location = row[3]
        if not number.is_nan(matricula) and matricula != "Total" and matricula != "Matrícula":
...
def parse(data, chave_coleta, month, year):
    ...
    # Puts all parsed employees in the big map
        employees.update(parse_employees(data.contracheque, chave_coleta, CONTRACHEQUE_2018))
    else:
